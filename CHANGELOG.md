...
### Fixed
- **Script editor no longer shows a blank box when the syntax highlighter overlay fails** — the editor used a layered pattern where `.script-textarea` was rendered with `color: transparent` and the only visible text came from a `.script-highlight` overlay positioned on top with `z-index: 1` and a solid background. If the overlay produced empty output for any reason (a JS error, a missed call, an edge-case input), the user saw a blank panel even though the textarea actually held the code. Inverted the layering: the textarea now renders its own text in `var(--text-primary)`, and the highlight overlay paints transparent-on-transparent with only the colored spans showing through. Syntax coloring still works; the editor degrades gracefully if the overlay is empty.
...
- **Data import** — `POST /api/data/import` restores from an export JSON. Imports jobs, variables, connections, groups, and settings. Skips existing items by name/ID rather than overwriting. Admin only.
- **Role-based UI hiding** — viewers no longer see Edit, Delete, Trigger, Copy, Create, or other write-action buttons that would 403. Builder nav link hidden for viewers. Variables are read-only for non-writers. Connections and scripts hide edit/delete. Pipeline schedule/trigger hidden. Bulk actions hidden. Demo mode (viewer role) gets a fully read-only UI automatically.
- **System theme auto-detect** — new installs default to system preference (light/dark) instead of forcing dark mode. Existing users with an explicit theme choice are unaffected.
- **Execution output search** — search on the Runs tab now searches stdout AND stderr (was stdout only). Execution detail modal has a new "Search in output" input that highlights matches across both output and error sections with match count.
- **Agent long-polling** — custom agents can use `?wait=30` on the poll endpoint for near-instant job dispatch. Server holds the connection and returns immediately when work is queued. Max 60s wait. Falls back gracefully for agents that don't use it. Standard agents remain push-based.
- **DAG task instance history** — dependency map nodes show a sparkline of the last 10 execution statuses (colored rectangles: green/red/blue/grey). Hovering a node shows a rich tooltip with job name, group, task type, and execution history blocks. Pipeline stage cards also show a mini history bar of last 8 runs.
- **Extending Kronforce docs** — new in-app docs section and `docs/EXTENDING.md` mapping 25+ Jenkins plugins to Kronforce equivalents. Covers custom agents, Rhai scripts, output rules, MCP server, REST API, and webhooks as extension points.
- **Agent node inventory** — agents report OS, architecture, CPU count, memory, and agent version on registration. Shown on agent cards in Settings. New migration `0020_agent_system_info.sql`. Backward compatible with older agents.
- **Optimistic concurrency control** — job updates accept optional `if_unmodified_since` timestamp. If the job was modified by another user since it was loaded, the server returns 409 Conflict. The UI auto-detects conflicts and reloads the latest version.
