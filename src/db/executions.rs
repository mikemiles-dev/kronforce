...
impl Db {
    ...
    pub fn insert_execution(&self, rec: &ExecutionRecord) -> Result<(), AppError> {
        ...
        let triggered_by_json = serde_json::to_string(&rec.triggered_by).map_err(|e| AppError::Internal(format!("serialize: {e}")))?;
        conn.execute(
    ...
    pub fn update_execution_extracted(
        ...
        conn.execute(
            ...
            params![serde_json::to_string(extracted).map_err(|e| AppError::Internal(format!("serialize: {e}")))?, id.to_string()],
        )
        .map_err(AppError::Db)?;
        Ok(())
    }
